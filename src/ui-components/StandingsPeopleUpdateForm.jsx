...
export default function StandingsPeopleUpdateForm(props) {
  const {
    id,
    standingsPeople,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...standingsPeopleRecord };
  ...
  const [standingsPeopleRecord, setStandingsPeopleRecord] =
    React.useState(standingsPeople);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(StandingsPeople, id)
        : standingsPeople;
      setStandingsPeopleRecord(record);
    };
    queryData();
  }, [id, standingsPeople]);
  React.useEffect(resetStateValues, [standingsPeopleRecord]);
  const validations = {
    userId: [],
    rank: [],
    points: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            StandingsPeople.copyOf(standingsPeopleRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "StandingsPeopleUpdateForm")}
    >
      <TextField
        label="User id"
        isRequired={false}
